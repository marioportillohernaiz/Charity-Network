...
export default function RejectRequest({ request, resourceData } : { request: TransitData; resourceData: ResourcesData[] }) {
  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const resource = resourceData.find(r => r.id == request.resource_id);

  const handleSubmit = async () => {
    const response = await rejectTransit(request.id);
    if (response.success) {
